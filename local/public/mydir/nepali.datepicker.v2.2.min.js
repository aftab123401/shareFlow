...
function getYearSelect(t, e, n) {
    ...
    for (i = Math.round(n / 2) > 0 && parseInt(e) - Math.round(n / 2) >= 2e3?parseInt(e) - Math.round(n / 2):2e3; i <= (Math.round(n / 2) > 0 && parseInt(e) + Math.round(n / 2) <= 2090?parseInt(e) + Math.round(n / 2):2090); i++)
        a += '<option value="' + i + '"' + (e == i ? " selected" : "") + ">" + englishNo2Nep(i) + "<li>"
    return a += "<select>"
}
...
function count_bs_days(t, e) {
    ...
    for (l = 0; r > l; l++)
        p -= this.bs[s][l]
    for (p += this.bs[s][11], l = o - 1; 12 > l; l++)
        p -= this.bs[d][l]
    return p -= i + 1, p += h - 1
}
